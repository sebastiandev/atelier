...
type LoopSelectorDialogProps = {
  ...
  onClose: () => void;
  onStart: (definition: LoopDefinition, briefNote?: string) => Promise<void>;
};
...
export function LoopSelectorDialog({
  ...
  async function start(definition: LoopDefinition) {
    ...
    setError(null);
    try {
      await onStart(definition, briefNote.trim() || undefined);
    } catch (err) {
...
function PlanningSlotBindings({
  ...
  const criteria = acceptanceCriteriaCount(target.content);
  const firstAgent = definition.stages.find((stage) => stage.agent !== null);
  return (
    <section className="loop-planning-bindings">
      <header><span>Task slot · bound automatically</span><em>all required slots bound · ready</em></header>
      <div className="loop-injected-row">
        <DocIcon size={11} />
        <strong>{target.artifact.title}</strong>
        <span>{target.artifact.id.toUpperCase()} · doc + {criteria} {criteria === 1 ? "criterion" : "criteria"}</span>
        <em className="tag info">injected</em>
      </div>
      <div className="loop-injected-row">
        <CheckIcon size={11} />
        <strong>Artifact report contract</strong>
        <span>structured step report schema</span>
        <em className="tag info">injected</em>
      </div>
      <label>
        <span>{firstAgent?.name ?? "Implementation"} · for this artifact <em>· optional</em></span>
        <textarea
          rows={2}
          value={note}
          onChange={(event) => onNote(event.target.value)}
          placeholder="Extra directives beyond the artifact document"
        />
      </label>
      <small>Injected inputs are read-only. Edit the artifact document to change them.</small>
    </section>
  );
}
