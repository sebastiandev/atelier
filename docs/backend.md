...
`domain/planning/` owns source-backed planning behavior. `POST /api/works/{slug}/plan/framework-status` checks whether BMAD, Spec-kit, or OpenSpec is initialized in the selected working folder; Planning does not continue when the selected framework is missing. If the user agrees to initialize it, `POST /api/works/{slug}/planning-setup-chat` creates/reuses a normal visible setup chat in the selected folder with a backend-built setup prompt and write-capable provider options. `POST /api/works/{slug}/planning-chat` creates/reuses the reserved real `Planning` chat, builds its first prompt in the backend, and upserts a SQL `PlanningSession` row with the selected root, framework, profile, artifact output folder, provider, model, and options. Discovery readiness is backend-gated: `domain/planning/readiness.py` accepts only the structured `atelier_planning_ready` marker, and `commands.planning.mark_ready` persists it under `chats.options.planning_readiness` while the WS route appends a `planning_readiness` event. `POST /api/works/{slug}/plan` is the public source-plan materialization command: it resolves materializer configuration from the persisted PlanningSession, creates or reuses an internal write-capable `Planning materializer` chat, builds `PlanningMaterializationPrompt`, runs the selected provider from the repo root, denies permission prompts for HTTP/browser/package-install/remote-fetch work, follows up when a turn ends without an `atelier_plan_materialization` report, parses that metadata report, and delegates to the internal manifest submission command. The materializer is prompted to generate the complete reviewable work package in one pass, including all known executable stories, tasks, spikes, bugs, hotfixes, or follow-up items with enough detail to assign directly to an implementation agent. Framework output folders are defined in `domain/planning/frameworks.py` (BMAD uses `_bmad-output/<WRK>/`); `.atelier/planning/<WRK>/` is reserved for Atelier-owned state like `manifest.json`. Manifest submission is metadata-only after the report is parsed: each artifact entry carries `path`, `title`, `artifact_kind`, `executable`, and path-based `dependencies` relative to the framework output folder, with no Markdown `content`. `FsPlanningFiles` stores state under `<root_path>/.atelier/planning/<WRK>/` and resolves source reads/writes through the manifest's `artifact_root_path`; `domain/planning/service.py` is now projection-only, indexing Markdown plus manifest metadata into a normalized Plan View with source refs, hashes, readiness, dependencies, launch blockers, runs, proposals, and tracking links. Planning mutations live in their corresponding `domain/commands/planning/*` use case modules and share small invariants/actions from `domain/planning/actions.py`. Artifact edits go through `PUT /api/works/{slug}/plan/artifacts/{id}` with an `expected_hash`; mismatches return 409 so users do not overwrite out-of-band Markdown edits. Approval stores the current source hashes in the manifest; this plan-level baseline is the acceptance path for non-executable source docs, and those approved docs can satisfy dependencies. Executable artifacts still become dependency-satisfying only after a run is completed pending review or accepted. Artifact run state lives in `manifest.json` under `artifact_runs`: `POST /runs` starts a durable `run-NNN`, sends the backend-owned run prompt, and schedules `commands.planning.run_monitor`; the monitor reads transcript messages after the stored cursor, extracts the latest single-line `atelier_loop_report` JSON marker, applies reports through `domain/loop`, creates pending source proposals from an optional `proposed_source` field, auto-sends continuation prompts for missing/invalid/incomplete reports, and stops when the run is completed, blocked, failed, or accepted. Repeated incomplete reports fail the loop once the definition's retry limit is reached. `POST /runs/{run}/resume` is valid only for `blocked_user` and schedules monitoring again after sending the blocker-resolution prompt. `POST /runs/{run}/accept` accepts only `completed_pending_review`/`completed` runs and writes `summaries/<artifact>-<run>.md`; `/runs/{run}/cleanup` records cleanup after the normal agent delete path removes transient workspace state. Loop assessment requires summary, divergences, skipped scope, blockers, decisions, changes, and validation evidence before a run becomes `completed_pending_review`; reported blockers become `blocked` with `loop_status=blocked_user`.
...
Schema v18 adds `loop_runs` and `loop_step_runs`. They store the immutable definition, current state, stage reports/cursors, timestamps, and persisted worker lease; the Planning manifest remains the source-backed UI projection. Monitors claim and renew the SQL lease, and startup reconstructs running Planning monitors from SQL. The manifest scan in `main.py` is an explicit import boundary for runs created before schema v18.
...
Schema v17 adds `planning_sessions`, one row per Work for the selected Planning setup. Schema v18 adds durable loop-run/stage state and recovery leases. Existing works without a Planning session return 404 from materialization until Planning is started or a session is backfilled; plan reads still use the source manifest.
