...
Every adapter waits for a permission decision through `infrastructure/agents/permission_wait.py` rather than a bare `await fut`. An unanswered prompt used to wedge the provider session permanently — no decision, no turn metrics, no trailing `idle` — and that is reachable whenever the prompt never renders (suppressed dialog, closed tab, a posture whose exit handshake nobody can answer). The wait is bounded at 30 minutes and then denies; auto-allowing would run a tool the user never approved. The deadline is deliberately generous because answering is a human action: this is a stuck-session backstop, not a politeness timer. Adapters already guard `resolve_permission` on `fut.done()`, so a late frame after expiry is ignored rather than crashing.
