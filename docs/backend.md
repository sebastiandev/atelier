...
Dynamic stage context is assembled when each provider prompt is sent. `previous_report` injects the configured predecessor's structured summary, findings, and validation evidence, including when a failed stage is retried in a fresh session; `changed_files` injects the latest reported path and line statistics; `workspace_diff` snapshots the live checkout's branch, HEAD, and porcelain status and directs the agent to inspect the complete patch locally. Only context kinds declared by the pinned stage are rendered, and filesystem references remain path indexes rather than copied file bodies. The configured predecessor is honored on both prompt paths — forward advance and resume resolve it through `actions.declared_previous_row` (`backend/src/domain/loop/actions.py:139`), so a trailing corrective stage never masks the substantive report a review is configured to read. User feedback is one record type, not five. `domain/loop/feedback.py` stores every request a run owes an answer to -- an approval's requested changes, a review gate's send-back, selected PR comments, and a retry note -- on `loop["feedback"]`, and what used to differ between those shapes is now data on the record: `scope` (`open` outlives the attempt, `attempt` ends with it), `target` (`restart` the pass or `retry` the stage), and `answered_by`, the stage whose pass discharges the request.
