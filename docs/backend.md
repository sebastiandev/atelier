...
Current ACP providers: `claude-acp` (`@agentclientprotocol/claude-agent-acp` 0.61.0) and `codex-acp` (`@agentclientprotocol/codex-acp` 1.1.7) are pinned in `backend/acp-runtime/package-lock.json` and spawned from `backend/acp-runtime/node_modules/.bin`; they are not fetched lazily with `npx`. `scripts/dev-backend.sh` installs or refreshes that runtime folder when it is missing or stale, and `/update` runs the same npm install when the ACP runtime lockfile changes. Codex ACP 1.x exposes `read-only`, `agent`, and `agent-full-access` modes; provider parsing still accepts the former `auto` and `full-access` values so persisted sessions remain resumable. Claude ACP's protocol-level `fast` option is exposed through Atelier's common `fast-mode` provider setting. `opencode` remains a local `opencode acp` binary. The bespoke `claude-code` / `codex` adapters are compatibility-only for existing stored agents and are not exposed for new sessions. Amp has no ACP path and is the only bespoke adapter offered for new work.
