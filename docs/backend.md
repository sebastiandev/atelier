...
ACP stdout is newline-delimited JSON and tool-result frames can exceed asyncio's 64 KiB default reader limit. The adapter starts provider subprocesses with a bounded 16 MiB stream limit so large inspection/test output remains a valid frame instead of surfacing as a misleading connection closure.
