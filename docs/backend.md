...
ACP subprocesses run in their own process group on POSIX so reconnect/close kills wrapper children too (`codex-acp` launches the real Codex binary plus MCP servers). This prevents orphaned provider subprocesses from surviving after transport recovery. The subprocess stdout reader uses a 50MB line buffer (`AcpAdapter._connect`) because ACP JSON-RPC frames can exceed asyncio's 64KB default during replay or multimodal turns; if this regresses, backend logs show `LimitOverrunError` / `chunk exceed the limit` before any transcript rows append.

A Claude ACP prompt failure matching exactly `Authentication required` is terminal (`infrastructure/agents/acp/adapter.py:751`). The Claude factory opts into this handling with its recovery command; Codex and OpenCode retain generic ACP error behavior. The adapter emits the normal `error` event enriched with `code="authentication_required"` and `recovery_command="claude auth login"`, then ends its event stream. Supervisor eviction closes the stale process; reconnect registers a replacement lazily, so no provider process starts until the user's next manual submission.

Current ACP providers: `claude-acp` (`@agentclientprotocol/claude-agent-acp`) and `codex-acp` (`@agentclientprotocol/codex-acp`) are pinned in `backend/acp-runtime/package-lock.json` and spawned from `backend/acp-runtime/node_modules/.bin`; they are not fetched lazily with `npx`. `scripts/dev-backend.sh` installs or refreshes that runtime folder when it is missing or stale, and `/update` runs the same npm install when the ACP runtime lockfile changes. `opencode` remains a local `opencode acp` binary. The first two **coexist** with the bespoke `claude-code` / `codex` adapters until parity is validated; removing the legacy pair is a deliberate follow-up. Amp has no ACP path and stays bespoke.
...
- **Restore**: `session/load` (replay suppressed — Atelier's transcript already has those turns) → `session/resume` → fresh session with a warning event, in that capability order. Some ACP agents can leak restored-history `session/update` frames just after load/resume resolves; the adapter suppresses assistant/thought/tool/plan/usage updates until the next real `prompt()` starts, while still accepting config metadata updates. If a restored ACP session accepts restore but then repeatedly closes on prompt delivery, the adapter reconnects once, then starts a fresh provider session and emits a new `session_established` event so the supervisor persists the replacement id instead of looping on the poisoned one.
