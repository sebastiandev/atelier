...
No SQLite migration is involved. Existing works with no planning-root pointer simply return 404 for `GET /plan` until planning is started. The feature is additive to `work.json` and wire shapes: legacy works, agents, chats, artifacts, and reconcile behavior are unchanged.
