...
A failed-stage retry uses a fresh provider transcript in the retained workspace. When the current pass was opened from pull-request feedback, `domain/loop/lifecycle.py` re-seeds the selected comments and user instructions from the durable pending-feedback bundle so the retry retains its purpose.

Dynamic stage context is assembled when each provider prompt is sent. `previous_report` injects the configured predecessor's structured summary, findings, and validation evidence, including when a failed stage is retried in a fresh session; `changed_files` injects the latest reported path and line statistics; `workspace_diff` snapshots the live checkout's branch, HEAD, and porcelain status and directs the agent to inspect the complete patch locally. Only context kinds declared by the pinned stage are rendered, and filesystem references remain path indexes rather than copied file bodies. The configured predecessor is honored on both prompt paths — forward advance and resume resolve it through `actions.declared_previous_row` (`backend/src/domain/loop/actions.py:139`), so a trailing corrective stage never masks the substantive report a review is configured to read. User feedback is durable for the pass that carries it rather than for one prompt: requested changes from final approval are stored by `domain/loop/pass_feedback.py` and PR comments by `pending_pr_feedback`, and both render into every agent stage of that pass, so reviews see the intent they are checking against.
