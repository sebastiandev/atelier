...
The shared run footer exposes **Open in editor** beside **Open transcript** as soon as the active stage workspace resolves. `openEditor.ts` picks the transport from the descriptor, never from the editor's name: one with a `url_template` is opened by the browser as it always was, one without (Emacs today) is launched by the backend. It takes the same target console does — the run when the run owns a workspace, otherwise the run's agent — so a command-launched editor lands exactly where a URL-handler one does, including on stages that never had an agent. Launch failures use the run's existing visible error surface. The action remains available during implementation, review, blocked, failed, and accepted states while that workspace path is retained. Active standalone and Planning runs also expose **Cancel run** there; cancellation preserves the run workspace and transcript.
...
- **Right** cell: `tile-controls` wrapper with **open-in-IDE / handoff / maximize / detach / close** `.btn.icon.sm` controls. The open-in-IDE button uses the selected editor descriptor from `GET /api/settings`. An editor with no `url_template` is launched by the backend (`POST /api/agents/{slug}/open-in-editor?editor=…`); the rest interpolate path tokens such as `{path_uri}` / `{path_param}` and let the browser route the protocol to its OS handler, so the page stays. Backend launch failures appear in the tile's existing toast surface.
...
- `settings.ts` — `{ theme, editor, terminal, accentHue, editorOptions, terminalOptions }`, hydrated from backend `GET /api/settings` and written through with `PUT /api/settings`. The backend owns the selectable editor/console descriptors (`label`, `command`, `url_template`); the FE renders them and interpolates path tokens for editor URL handlers. `opensFromUrl()` reads whether a descriptor has a template at all, which is what `openEditor.ts` branches on — so a command-only editor never falls through to the VS Code default, and adding another is a descriptor entry rather than a frontend change. Legacy `atelier:tweaks` / `atelier:theme` localStorage blobs are migrated into the backend once on boot.
