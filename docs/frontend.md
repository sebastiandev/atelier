...
**⌘K search (`SearchModal.tsx`).** Three groups — projects, work units, loop runs — over one flat keyboard list. Matching stays client-side substring; there is no search backend. Projects and works come in as props, but **loop runs are fetched by the modal itself** (`listRecentRuns`), because the modal has four mount points (`App`, `Home`, `ProjectScreen`, `WorkView`) and prop-drilling would touch them all. That fetch is fire-and-forget: a slow or failing `/api/runs` must never keep projects and works from rendering.

A run row reads story-first — `source_ref` in the mono id slot, the goal as the name, the story title as the subtitle, the work name in the tail — falling back to the work when no story triggered it. Enter navigates to `/works/{slug}?run={id}`, carrying `&artifact=` for a planning run so it lands on the run view without passing through the work overview. `WorkView` consumes those params on mount and strips them with `replaceState`, exactly as `?chat=` and `?mode=` are handled; an id that no longer resolves leaves the overview showing rather than erroring. Loop-mode runs have no artifact and are seeded through `LoopMode`'s `initialRunId` instead, which falls back to the newest run when unresolved.

With no query typed, runs and works are ordered newest-first — recency is the empty-query default, not a ranking function, so once a term is typed match order stands. Works sort on `created_at`, the only timestamp `WorkSummary` carries, so that is "recently created" and not "recently touched". Matching, ordering and the deep-link parser live in `searchRanking.ts` so they are unit-testable away from the component.

**Composer pickers (`/` and `@`).** One popover recipe, two triggers (`ComposerPicker.tsx`, spec §06 / `design/Composer Picker Handoff.md`). `/` lists the agent's advertised commands (`session_commands`), `@` lists plan references; same container, header, row grid, footer and keyboard contract — only the contents and the row's mono/prose order differ, driven by `[data-trigger]` in CSS. The type rule: **what you insert is mono, what you read is UI type**, so commands put the token on the primary line and references put the title there. Search lives in the textarea, never in a field inside the popover: terms split on `/`, `-` and whitespace, every term must hit, matched runs get the accent mark in both lines (`pickerSearch.ts` — pure, unit-tested; the JSX half is `markMatches` in the component). Filter chips narrow the searched set and hide at 0 count.

`useCommandPicker` wires the `/` half into all three composers — the agent tile, the chat tile (which is what planning chat and loop discuss both render), and the standalone chat route — because two composers with differently-behaved pickers would be worse than none. Three rules matter. The trigger is stricter than `@`: position 0 only, closing on the first space, so `src/foo` and `and/or` never trigger it — the slash is the discriminator, not the leading character, so a `$` prefix is allowed (codex-acp namespaces plugin commands as `$plugin:command`). Esc stays reserved for stop-agent — a picker consumes it *only while open*. And send is guarded by `unknownCommandName`: providers parse the slash themselves and silently no-op a name they don't know, producing an empty turn indistinguishable from a hung agent, so an unadvertised name is refused inline. Gating is on `commands.length`, never on a provider name.

The command **origin** axis (PRJ / USR / OC chips) is plumbed but dormant: `AvailableCommand` carries only name/description/input, and `_meta` comes back empty from opencode, claude-acp and codex-acp alike, so `origin` parses to null and the 0-count chips hide themselves. The axis lights up the moment a provider reports one.
...
## Story agents (agent-mode story view)

`PlanningMode` does not own agents: `WorkView` passes `agents`, the closed set, focus, and a `renderAgentTile(agent)` callback (the same `AgentTile` wiring the work canvas uses, extracted to one const in `WorkView.tsx`). The story aside's **Work this story** panel offers `Start loop` and `New agent`; the latter opens the shared `NewAgentDialog` with the story `.md` as a `lockedContexts` chip (no remove control) and the plan sources as ordinary `initialContexts`, plus `artifactId`. Once a story has `work_mode === "agents"` or any agent, `ArtifactDetail` renders `.pm-story-canvas` instead of the aside: doc column (default 404px, resizable 320–760 through `PaneResizeHandle` into `useLayoutStore.planningDocWidth`, collapsible to a strip via `planningDocCollapsed`; it yields before the agent field drops under 360px) · `StoryAgentCanvas` (`StoryAgentCanvas.tsx`, dnd-kit grid keyed `${work}:${story}` in `state/agentOrder.ts`) · tracked-PR column (238px). `PlanRail` nests a focused story's agents beneath it (`.pm-tree-agent`, click restores a closed tile) and collapses other stories' agents to an `N agents` chip.

**Mark done.** The canvas header carries `✓ Mark done` (hidden once the story reads `done`) and, when every tracked PR is merged, the hint "all N PRs merged · mark done?". It opens `MarkStoryDoneDialog.tsx`: summary prefilled from the tracked PRs, optional "close the tiles to the rail". `WorkView` calls `acceptPlanArtifact`, closes the tiles through the closed-store, and refreshes the plan. Done is never automatic — with several agents there is no single signal.

**Tracked PRs.** In agent mode the right column lists every PR of the story: artifact rows whose `artifact_id` matches (opened by an agent; the card shows the opener's pip and slug, or "removed") and loop-run PRs (`⟳ run N · loop`, which open the run). Clicking an agent PR replaces the grid with `StoryPrView.tsx`: it polls `getPrArtifactView` every 60 s, renders the description card and GitHub-style threads (grouped by `prCommentThreads` from `PrLifecyclePanel.tsx`), and its footer sends the selected threads to the opener as Implement or Discuss via `sendPrArtifactFeedback`. The target is a static chip, never a dropdown; when the opener was removed it reads "will relaunch" and the parent's agent list is refreshed after the send.
