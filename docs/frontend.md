...
The shared run footer exposes **Open in editor** beside **Open transcript** as soon as the active stage workspace resolves. The centralized transport selector in `src/emacsEditor.ts` sends Emacs through the agent-slug-only backend endpoint, whose non-waiting `emacsclient -n` launch returns without holding the request open until the frame closes; every other editor keeps its descriptor URL navigation. Emacs is offered only when the selected run workspace is agent-backed, and launch failures use the run's existing visible error surface. The action remains available during implementation, review, blocked, failed, and accepted states while that workspace path is retained. Active standalone and Planning runs also expose **Cancel run** there; cancellation preserves the run workspace and transcript.
...
- **Right** cell: `tile-controls` wrapper with **open-in-IDE / handoff / maximize / detach / close** `.btn.icon.sm` controls. The open-in-IDE button uses the selected editor descriptor from `GET /api/settings`. Emacs dispatches by agent slug to `POST /api/agents/{slug}/open-in-editor`; URL-based editors interpolate path tokens such as `{path_uri}` / `{path_param}` and let the browser route the protocol to its OS handler. Failures from the Emacs endpoint appear in the tile's existing toast surface.
...
- `settings.ts` — `{ theme, editor, terminal, accentHue, editorOptions, terminalOptions }`, hydrated from backend `GET /api/settings` and written through with `PUT /api/settings`. The backend owns the selectable editor/console descriptors (`label`, `command`, `url_template`); the FE renders them and retains the historical VS Code fallback for missing URL templates. `emacsEditor.ts` separates Emacs before URL rendering and chooses its HTTP transport for both agent tiles and run footers, preventing that fallback from routing Emacs to VS Code. Legacy `atelier:tweaks` / `atelier:theme` localStorage blobs are migrated into the backend once on boot.
