...
`plan.planning_path` is the Atelier-owned state folder (`.atelier/planning/<WRK>`). Editable source artifacts live under `plan.plan_artifacts_path`, which defaults from the backend framework definition but can be overridden in Planning setup, and artifact `path` values are relative to that folder.
