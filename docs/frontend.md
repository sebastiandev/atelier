...
**A per-run timeout override was proposed and rejected.** `design/Loop Brief Handoff.html` §02 puts the timeout in run setup as a run-level override, tagged `run override` and pinned with the brief. It is not built and should not be: a timeout belongs to the stage, not to the run that happens to execute it. Nothing supports it either — a brief carries `provider`, `model` and `options` only (`LoopBriefAgent`), so there is no field to write, and adding one would mean a wire change, a domain change and a resolution rule, to make a stage property configurable in a place that does not own it. The handoff's underlying rule still holds and is implemented: wherever the timeout *is* editable, it renders beside the agent and effort that determine it.
