...
`useCommandPicker` wires the `/` half into all three composers — the agent tile, the chat tile (which is what planning chat and loop discuss both render), and the standalone chat route — because two composers with differently-behaved pickers would be worse than none. Three rules matter. The trigger is stricter than `@`: position 0 only, closing on the first space, so `src/foo` and `and/or` never trigger it. Esc stays reserved for stop-agent — a picker consumes it *only while open*. And send is guarded by `unknownCommandName`: providers parse the slash themselves and silently no-op a name they don't know, producing an empty turn indistinguishable from a hung agent, so an unadvertised name is refused inline. Gating is on `commands.length`, never on a provider name.
