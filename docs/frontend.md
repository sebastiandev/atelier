...
**The timeout renders in the run-configuration block, as the stage's value.** `design/Loop Brief Handoff.html` §02's rule is that a timeout never renders away from the agent and effort that determine it, so it appears here as well as in the stage editor. It is display-only: the value belongs to the stage. What was rejected is a per-run override *field* on the brief — `LoopBriefAgent` carries `provider`, `model` and `options` only, and adding a timeout to it would make a stage property configurable in a place that does not own it.
