...
- `LoopRunStateStore` and `LoopWorkingRootRepository` (`backend/src/domain/loop/ports.py`) — feature-neutral run-state and Work-root boundaries. Standalone Loop uses the SQL objective adapter; Planning adapts its manifest and persisted session without exposing Planning types to Loop.
...
- A command never calls another command. Shared validation, transitions, runtime behavior, or persistence belongs in a domain action/function that each command invokes with its own inputs.
...
**Planning docs are source-backed Markdown.** The Atelier-owned planning manifest lives at `~/Atelier/works/<work-slug>/planning/manifest.json` (a known path, so nothing needs a pointer to find it); framework *source* docs live in the user-selected folder. Framework source files live under the manifest's `plan_artifacts_path`; BMAD defaults to `_bmad-output/<work-slug>/`, and Planning setup can override that folder with a root-contained path such as `bmad/<work-slug>`. Output folder and file names are paths only and never select or imply a planning framework; the persisted `PlanningSession.framework` is authoritative. The selected planning setup is persisted as one SQL `PlanningSession` per Work; the Planning chat remains the transcript/runtime, not the materialization source of truth. Framework readiness is repo-local: BMAD, Spec-kit, and OpenSpec must be initialized in the selected folder before Planning starts; optional setup uses a normal backend-prompted setup chat, not a hidden command runner. Planning prompts are backend-owned through `domain/prompts/build.py`; the read-oriented `Planning` chat handles discovery. Public `POST /plan` owns source-plan materialization end to end: it resolves root/framework/profile/provider/model/options from the persisted PlanningSession, creates or reuses an internal write-capable `Planning materializer` chat, lets the selected framework write files, loops until an `atelier_plan_materialization` metadata report is present, and then indexes the files through an internal metadata-only manifest submission command. The generic chat runtime accepts an opaque system-prompt override, while Planning owns the materializer system, fresh-session recovery, and report-repair prompts. The public request never carries artifact metadata or Markdown content. The manifest records framework/profile/depth, root path, artifact root, artifact metadata, source hashes, artifact runs, review proposals, and lightweight tracking links; the Plan View is a projection built by `domain/planning/service.py`, not a persisted table.

**Loop is the execution primitive; Planning is a client.** `domain/loop/` owns definition/run entities, ports, provider policy, prompts, state transitions, lifecycle actions, persistence projection, leases, and the monitor. Standalone Loop commands adapt the user-selected goal/root/provider/model/options plus template into those actions. Planning commands resolve artifact metadata and persisted PlanningSession defaults, then adapt the artifact manifest through `PlanningLoopRunStore`; they never provide an execution engine to Loop. The Planning monitor command is only a dependency-wiring adapter over the same Loop monitor. Definitions are one self-contained `loop.yaml` (instructions inline) under `~/Atelier/loops/<id>/`, and every run persists a pinned snapshot through SQL. Resolution is Work overlay over library over built-in. Repositories only load/store state; commands choose inputs and invoke actions.
...
**Bump the ACP wrappers and the Python SDK together.** The wrappers emit ACP session updates; `agent-client-protocol` (Python) parses them. That parse is a *discriminated union with no fallback* — an unrecognised `sessionUpdate` tag raises `ValidationError` inside the SDK, before the frame ever reaches `mapping.SessionMapper.handle`, whose deliberate "ignore anything newer than us" branch therefore cannot protect the pump. So a wrapper new enough to emit a tag the pinned SDK lacks breaks the stream rather than degrading. Verified at the 0.61→0.70 / 1.1.7→1.4.0 bump: the wrappers gained plan/goal frames and SDK 0.12.1 added the matching `plan_update` and `plan_removed` tags that 0.10.1 did not have.

A second thing only the type checker catches: `AcpConnection` in `acp/adapter.py` is a structural Protocol mirroring `ClientSideConnection`. Every call site passes keyword arguments, so SDK parameter *reordering* changes nothing at runtime and shows up solely as a mypy assignment error. Run `mypy src/infrastructure/agents/acp/` after any SDK bump; the test suite will not catch it.

**ACP npm wrappers are backend runtime dependencies.** Claude/Codex ACP wrappers are pinned under `backend/acp-runtime/package-lock.json`, installed with `npm install --prefix backend/acp-runtime`, and spawned from `backend/acp-runtime/node_modules/.bin`. They should not be fetched dynamically from provider code with `npx`; update the lockfile when bumping wrapper versions; `./atelier update` installs them for everyone else.
