...
- **Work hero actions** keep the Work action labeled but neutral (`Mark done` / `Reopen` via `.work-hero .pills .btn`) and make secondary chat/move/delete actions icon-only 28px buttons (`.btn.icon.sm`) with tooltips. The delete icon uses the muted danger treatment, not a full primary-danger fill. Keep the colored primary treatment for canvas-level creation actions such as `New agent`.
- **Work rail scrolling** keeps Shared folders, Active agents, Chats, and Pull requests as separate scroll bodies when they overflow. Shared folders only shows project shares; generated chat seed/context folders stay accessible from the owning chat tile. Rail scrollbars use a subtle neutral thumb from the current theme rather than browser defaults or project/persona accents. The Pull requests header can open a compact local filter for title, slug, status, source, and URL metadata.
