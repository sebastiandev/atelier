...
class AcpConnection(Protocol):
    ...
    Parameter *order* mirrors ``ClientSideConnection`` exactly even though
    every call site passes keywords: a Protocol is structural, so a
    reordered positional parameter makes the real connection stop
    satisfying it and mypy rejects the assignment. Runtime is unaffected
    either way, which is precisely why the type checker is the only thing
    that catches SDK signature drift here.
    """
