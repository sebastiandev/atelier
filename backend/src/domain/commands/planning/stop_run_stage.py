"""Stop the running stage of a Planning artifact run, leaving it retryable."""
...
class StopArtifactRunStageRequest:
    """Command input for stopping one Planning artifact run's stage."""
...
async def execute(
    ...
    req: StopArtifactRunStageRequest,
) -> PlanArtifactDetail:
    """Stop the running stage, leaving the run retryable from that stage.

    The stage-level counterpart to ``cancel_run``: identical plumbing, but
    ``lifecycle.stop_stage`` fails only the running stage so the existing
    failure panel offers retry and its model/effort overrides.

    Preconditions: Work, artifact, and a run with a stage running exist.
    Postconditions: provider runtimes are released and the stage is failed
    with a reason naming the manual stop.
    """
