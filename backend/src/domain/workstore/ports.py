...
class WorkStore(Protocol):
    ...
    def set_agent_model(self, agent_slug: str, model: str) -> None:
        """Persist the agent's current provider model/primary selector.

        Used by live session config changes so detach/resume uses the
        same model the user selected in the running tile.
        """
        ...
    ...
    def rename_agent(self, agent_slug: str, name: str) -> Agent:
        ...
        split). Returns the updated agent. Raises ``ValueError`` if the
        slug doesn't resolve to an existing agent.
        """
        ...
    ...
    def update_pr_artifact_lifecycle(self, slug: str, lifecycle: dict[str, Any]) -> None:
        """Persist the story-PR tracking state on a PR row. No-op if gone."""
        ...

    def update_artifact_agent(self, slug: str, agent_slug: str | None) -> None:
        """Re-point an artifact's opening agent (a relaunched opener)."""
        ...
