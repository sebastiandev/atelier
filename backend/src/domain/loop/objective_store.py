"""Standalone objective adapter for the generic loop run state interface."""
...
OBJECTIVE_TARGET_ID = "objective"
OBJECTIVE_WORKTREE_SLUG = "loop"


def objective_run_key(run_id: str) -> str:
    """Return the stable repository key for one standalone run id."""
    return actions.loop_run_id(OBJECTIVE_TARGET_ID, "", run_id=run_id)


def get_objective_run(
    repository: LoopRunRepository,
    work_slug: str,
    run_id: str,
) -> LoopRunRecord | None:
    """Return one objective record, excluding other loop target kinds."""
    record = repository.get(work_slug, objective_run_key(run_id))
...
def _with_retained_workspace(
    ...
    if Path(workspace).name != OBJECTIVE_WORKTREE_SLUG or source_id is None:
        return record
    source = repository.get(record.work_slug, objective_run_key(source_id))
    ...
    if not source_workspace or Path(source_workspace).name == OBJECTIVE_WORKTREE_SLUG:
        return record
...
class ObjectiveLoopRunStore:
    """Load and save standalone objective runs from the loop repository."""

    def __init__(self, repository: LoopRunRepository) -> None:
        self._repository = repository

    def load(self, work_slug: str, run_id: str) -> LoopRunTarget | None:
        """Return one objective run snapshot without Planning projection."""
        record = get_objective_run(self._repository, work_slug, run_id)
        if record is None:
            return None
